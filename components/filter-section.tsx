...
import { BOUQUET_TYPE_NAMES, BOUQUET_SIZE_NAMES, FLOWER_COLOR_NAMES, FLOWER_KIND_NAMES } from "@/lib/constants"
...
export function FilterSection() {
  ...
  const handleKindToggle = (kind: number) => {
    setSelectedKinds((prev) => {
      const updated = prev.includes(kind) ? prev.filter((k) => k !== kind) : [...prev, kind]
  ...
  return (
    <section className="bg-muted py-16" id="filter">
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold text-foreground">{t("filter.title")}</h2>
        ...
        <div className="rounded-lg bg-card p-6 shadow-sm">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 ">
            ...
            <Select
              ...
              onValueChange={(value) => handleFilterChange("bouqetType", value ? Number.parseInt(value) : undefined)}
              >
              <SelectTrigger>
            ...
            <Select
              ...
              onValueChange={(value) => handleFilterChange("size", value ? Number.parseInt(value) : undefined)}
              >
              <SelectTrigger>
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2">
            <div>
              <h3 className="mb-3 font-medium text-foreground">{t("filter.colors")}</h3>
              <div className="flex flex-wrap gap-2">
                {Object.entries(FLOWER_COLOR_NAMES).map(([value, name]) => (
                  <Badge
                    key={value}
                    variant={selectedColors.includes(Number.parseInt(value)) ? "default" : "outline"}
                    className="cursor-pointer"
                    onClick={() => handleColorToggle(Number.parseInt(value))}
                  >
                    {name}
                    {selectedColors.includes(Number.parseInt(value)) && <X className="ml-1 h-3 w-3" />}
                  </Badge>
                ))}
              </div>
            </div>
            ...
            <div>
              <h3 className="mb-3 font-medium text-foreground">{t("filter.flower_types")}</h3>
              <div className="flex flex-wrap gap-2">
                {Object.entries(FLOWER_KIND_NAMES).map(([value, name]) => (
                  <Badge
                    key={value}
                    variant={selectedKinds.includes(Number.parseInt(value)) ? "default" : "outline"}
                    className="cursor-pointer"
                    onClick={() => handleKindToggle(Number.parseInt(value))}
                  >
                    {name}
                    {selectedKinds.includes(Number.parseInt(value)) && <X className="ml-1 h-3 w-3" />}
                  </Badge>
                ))}
              </div>
            </div>
