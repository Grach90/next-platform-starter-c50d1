...
export function BasketSidebar() {
  const { state, setOpen, removeItem, updateQuantity, clearBasket, getTotalPrice } = useBasket()
  const { t } = useLanguage()
    ...
    const message =
      state.items.length > 0
        ? `Hello! I'm interested in these flowers:\n${state.items
            .map(
              (item) =>
                `- ${item.flowerName} (Size: ${BOUQUET_SIZE_NAMES[item.size as keyof typeof BOUQUET_SIZE_NAMES]}, Qty: ${item.quantity}, Price: $${(item.price * item.quantity).toFixed(2)})`,
            )
  ...
  const handleQuantityChange = (flowerId: string, size: number, newQuantity: number) => {
      ...
      <SheetContent className="w-full max-w-md bg-sidebar text-sidebar-foreground">
        <SheetHeader className="border-b border-sidebar-border pb-4">
          <div className="flex items-center justify-between">
            <SheetTitle className="text-sidebar-foreground">{t("basket.title")}</SheetTitle>
            <Button
              variant="ghost"
              size="icon"
        ...
        <div className="flex flex-1 flex-col py-6 pr-3 pl-3">
          {state.items.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center text-center">
              ...
              <h3 className="mb-2 text-lg font-semibold">{t("basket.empty")}</h3>
              <p className="text-sm text-sidebar-foreground/70">{t("basket.empty_description")}</p>
            </div>
          ) : (
            <>
                ...
                {state.items.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-sidebar-accent-foreground truncate">{item.flowerName}</h4>
                      <p className="text-sm text-sidebar-accent-foreground/70">
                        Size: {BOUQUET_SIZE_NAMES[item.size as keyof typeof BOUQUET_SIZE_NAMES]}
                      </p>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => handleRemoveItem(item.flowerId, item.size)}
                      ...
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => handleQuantityChange(item.flowerId, item.size, item.quantity - 1)}
          ...
          <Button onClick={handleWhatsAppRedirect} className="w-full bg-green-600 text-white hover:bg-green-700 mt-4">
