...
export function GroupCardsSection() {
  ...
  const loadGroups = async () => {
    try {
      setLoading(true)
      const result = await ApiService.getAllFlowerGroups()
      setGroups(result)
    } catch (err) {
      ...
      <section className="py-16" id="groups">
        <div className="container mx-auto px-4">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-foreground">Flower Collections</h2>
            <p className="mt-2 text-muted-foreground">Explore our curated flower groups</p>
          ...
          {groups.map((group) => (
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                <img
                  src={group.imageLink || "/placeholder.svg?height=300&width=300&query=flower collection"}
                  alt={group.name}
                ...
                <div className="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 hidden md:block">
                  <h3 className="text-lg font-bold text-white drop-shadow-lg">{group.name}</h3>
                </div>
