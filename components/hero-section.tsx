  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute bottom-20 z-20 items-center flex flex-col items-center left-1/2 -translate-x-1/2 block md:hidden">
        <Button className="rounded-full bg-blue-400 w-60 h-10 text-lg">
          <a href="#filter">
            S E A R C H
          </a>
          </Button>
        <Button className="rounded-full bg-blue-400 w-60 h-10 text-lg mt-3">
          <a href="#groups">
            ORDER / CATALOG
          </a>
          </Button>
        <Button className="rounded-full bg-blue-400 w-50 h-9 text-sm mt-3">
          <a href="#info">
            About Us/Delivery/Payment
          </a>
        </Button>
        <Button className="rounded-full bg-gray-400 w-60 h-9 text-2xl mt-3 border-2 border-green-500"
        onClick={()=>window.open(`https://wa.me/${WHATSAPP_NUMBER.replace("+", "")}`)}
        > Chat
        </Button>
        <Button className="absolute bottom-0 -left-1 px-0 bg-transparent pointer-events-none"
        onClick={()=>window.open(`https://wa.me/${WHATSAPP_NUMBER.replace("+", "")}`)}
        >
